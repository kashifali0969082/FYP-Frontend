...
const useTextSelection = (onTextHighlight ) => {
...
const [first, second] = newSelected.map(id => {
  const item = gameData.find(item => item.id === id);
  if (!item) {
    throw new Error(`Item with id ${id} not found in gameData`);
  }
  return item;
});
  ...
  const cards = useMemo(() => {
    try {
      ...
      if (Array.isArray(content) && content.length > 0 && typeof content[0] === 'object') {
        const flashcardObjects = content ;
...
export const ToolOverlay= ({ toolType, content, onClose, onMinimize, onTextHighlight }) => {
